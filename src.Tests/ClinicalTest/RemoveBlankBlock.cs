...
public class RemoveBlankBlocksTest
{
    private readonly ClinicalTest _sut;

    public RemoveBlankBlocksTest()
    {
    ...
    public async void RemoveBlankBlocks(List<Block> expected, List<Block> mockedBlocks, params ClinicalTest[] clinicalTests)
    {
        ...
        List<Block> blocks2 = new();
        List<Block> expected2 = new List<Block>() {
        ...
        foreach (ClinicalTest c in clinicalTests)
        {
            await c.RemoveBlankBlock(mockedBlocks[0]);
            blocks2 = await c.GetBlankBlocks();
        }

        var serializedExpected2 = JsonConvert.SerializeObject(expected2);
        var serializedActual2 = JsonConvert.SerializeObject(blocks2);

        Assert.Equal(serializedExpected2, serializedActual2);
    }

    //[Fact]
    //public async void RemoveBlankBlockNothingRemoved()
    //{
